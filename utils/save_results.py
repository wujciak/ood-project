...
def plot_roc_curves(results, output_dir="results"):
    ...
    for ood_key in ood_keys:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"roc_{ood_key}.png"), dpi=200, bbox_inches="tight")
        plt.close()
...
def plot_pr_curves(results, output_dir="results"):
    ...
    for ood_key in ood_keys:
        ...
        for method in results["ID"].keys():
            ...
            plt.plot(recall, precision, label=f"{method} (AUPR={aupr:.3f})", linewidth=2)
        n_id = len(id_scores[list(id_scores.keys())[0]])
        n_ood = len(results[ood_key][list(results[ood_key].keys())[0]])
        baseline = n_ood / (n_id + n_ood)
        ...
        plt.tick_params(labelsize=11)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"pr_{ood_key}.png"), dpi=200, bbox_inches="tight")
        plt.close()
...
def plot_auroc_bar(auroc_df, aupr_df, output_dir="results"):
    ...
    metric_pairs = [(auroc_df, "AUROC"), (aupr_df, "AUPR")]

    for ax, (df, metric_name) in zip(axes, metric_pairs):
        for i, ood_key in enumerate(ood_keys):
            vals = [
                df.loc[(df["method"] == m) & (df["dataset"] == ood_key), df.columns[2]].values[0]
                for m in methods
            ]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "auroc_aupr_bar.png"), dpi=200, bbox_inches="tight")
    plt.close()
