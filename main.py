...
from utils.save_results import save_scores_to_csv, plot_score_histograms, plot_roc_curves
...
def main():
    ...
    for method in ['entropy', 'energy', 'hybrid', 'mc_dropout', 'ensemble']:
        ...
        for dataset_name, ood_scores in [('OOD_Cross', results['OOD_Cross'][method]),
                                         ('OOD_Synth', results['OOD_Synth'][method])]:
            y_true = np.concatenate([np.zeros(len(id_scores)), np.ones(len(ood_scores))])
            y_scores = np.concatenate([id_scores, ood_scores])
            auroc_summary.append({'method': method, 'dataset': dataset_name,
                                  'auroc': roc_auc_score(y_true, y_scores)})
            aupr_summary.append({'method': method, 'dataset': dataset_name,
